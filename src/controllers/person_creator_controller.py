...
class PersonCreatorController(PersonCreatorControllerInterface):
    ...
    def __validate_first_and_last_name(self, first_namee: str, last_name: str) -> None:
        # Expressão Regular para caracteres que não são letras
        non_valid_caracters = re.compile(r'[^a-zA-Z]')

        if non_valid_caracters.search(first_namee) or non_valid_caracters.search(last_name):
